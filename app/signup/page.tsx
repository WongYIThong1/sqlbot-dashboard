...
export default function SignupPage() {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!response.ok) {
                showErrorToast("Sign up failed", payload?.message ?? "Please try again later.");
                return;
            }

            showSuccessToast("Sign up successful", "Redirecting to login...");
    ...
    useEffect(() => {
        // reserved for future side-effects after mount
    }, []);
        ...
        <div className="min-h-screen w-full bg-black text-white flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none">
                <motion.div
                    ...
                    className="absolute top-[-10%] right-[-10%] w-[40%] h-[40%] bg-white/5 rounded-full blur-[100px]"
                />
                <motion.div
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ duration: 1.5, ease: "easeOut", delay: 0.2 }}
                    className="absolute bottom-[-10%] left-[-10%] w-[40%] h-[40%] bg-white/5 rounded-full blur-[100px]"
                />
            </div>
            ...
            >
                ...
                <form className="space-y-6" onSubmit={handleSubmit}>
                    ...
                    <motion.div
                        ...
                        className="space-y-2"
                    >
                        <label htmlFor="licenseKey" className="text-sm font-medium text-zinc-300 block">
                            License Key
                        </label>
                        <input
                            id="licenseKey"
                            ...
                            className="w-full bg-zinc-900/50 border border-zinc-800 rounded-lg px-4 py-3 text-white placeholder:text-zinc-600 focus:outline-none focus:ring-2 focus:ring-white/20 focus:border-white/20 transition-all duration-200 font-mono"
                            required
                            disabled={isSubmitting}
                        />
                    </motion.div>
